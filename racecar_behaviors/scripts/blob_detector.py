...
class BlobDetector:
    ...
    def image_callback(self, image, depth, info):
        ...
        if closestObject[2] > 0:
            ...
            rospy.loginfo("Object detected at [%f,%f] in %s frame! Distance and direction from robot: %fm %frad.", transMap[0], transMap[1], self.map_frame_id, distance, angle)
            msg = Float32MultiArray()
            # msg.data = [transMap[0],transMap[1],distance,angle*180.0/np.pi]
            msg.data = [transMap[0],transMap[1],distance,angle]
